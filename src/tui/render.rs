...
use super::App;
use super::dialogs::{
    render_agents_dialog, render_status_dialog, render_themes_dialog, render_which_key,
};
...
fn render_sessions_dialog(
    frame: &mut Frame<'_>,
    area: Rect,
    dialog: &super::app::SessionsDialogState,
        ...
        filtered
            ...
            .map(|(rel_idx, session)| {
                ...
                let badge = if is_active {
                    "[active]".to_string()
                } else if session.status == crate::persistence::SessionStatus::Running {
                    "[running]".to_string()
                ...
                let prefix_len = cursor.len() + active_dot.len() + title_prefix.len();
